...
class TestSaleCommission(TransactionCase):
    ...
    def _check_full(self, agent, commission, period, initial_count):
        ...
        self.assertIn(sale_order.invoice_ids.payment_state,
                      ["in_payment", "paid"])
        self._settle_agent(agent, period)
        settlements = self.settle_model.search([("state", "=", "settled")])
    ...
    def test_sale_commission_gross_amount_payment_annual(self):
        self._check_full(self.agent_annual,
                         self.commission_section_paid, 12, 0)
    ...
    def test_negative_settlements(self):
        ...
        commission_refund = self.env["account.move"].browse(
            action["domain"][0][2])
    ...
    def test_negative_settlements_join_invoice(self):
        ...
        action = wizard.button_create()
        commission_invoice = self.env["account.move"].browse(
            action["domain"][0][2])
    ...
    def test_commission_single_invoice(self):
        ...
        self.assertEqual(2, len(settlements))
        settlements.make_invoices(
            self.journal, self.commission_product, grouped=True)
