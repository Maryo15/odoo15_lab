...
class Appointment(models.Model):
    ...
    date = fields.Datetime(string='Fecha de entrega', default=lambda s: fields.Datetime.now(),
    ...
    physician_id = fields.Many2one('res.partner', string='Doctor encargado', select=True)
    ...
    def confirm_appointment(self):
        ...
        message_body = "Dear " + self.patient_id.partner_id.name + "," + "<br>Your Appointment Has been Confirmed " \
    ...
    def open_lab_request(self):
        for rec in self:
            lab_request = self.env['lab.request'].search([('lab_request_id','=',rec.name)]).ids
            view_id = self.env.ref('medical_lab_management.view_lab_request_tree').id
            return {
                'name': 'Lab Request',
                "type": "ir.actions.act_window",
                "res_model": "lab.request",
                "views": [[False, "tree"], [False, "form"]],
                "domain": [["id", "in", lab_request]],
            }
...
class LabPatient(models.Model):
    ...
    app_count = fields.Integer(compute="_compute_state",
