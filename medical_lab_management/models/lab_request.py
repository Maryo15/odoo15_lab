...
class LabRequest(models.Model):
    ...
    @api.onchange('test_request')
    def _onchange_(self):
        for rec in self:
            if rec.test_request:
                rec.request_line = rec.test_request.test_lines
    ...
    request_line_result = fields.One2many('lab.test.attribute.result', 'test_request_reverse', string="Test Lines")
    ...
    @api.onchange('test_request')
    def _onchange_test_request(self):
        for rec in self:
            if rec.test_request:
                rec.request_line_result = [(5, 0)]
                for line in rec.request_line:
                    values = {
                                'test_content': line.test_content.id,
                                'unit': line.unit.id,
                                'interval':line.interval,
                                #'result':line.result,
                                'test_request_reverse':rec.test_request.id
                            }
                    
                    rec.request_line_result = [(0,0, values)]

class LabTestAttributeResult(models.Model):
    _name = 'lab.test.attribute.result'
    _inherit = ['lab.test.attribute']

    result_request = fields.Char('Result Request')